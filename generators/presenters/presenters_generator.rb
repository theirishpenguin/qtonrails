class PresentersGenerator < Rails::Generator::Base

  def manifest
    record do |m|
      qforms_dir = "#{RAILS_ROOT}/vendor/plugins/qtonrails/app/qforms"
      ui_files = Dir["#{qforms_dir}/*.ui"]
      ui_proxies_dir = "#{RAILS_ROOT}/vendor/plugins/qtonrails/app/ui_proxies"
      presenters_dir = "#{RAILS_ROOT}/vendor/plugins/qtonrails/app/presenters"
      ...
      ui_files.each do |ui_f|
        ...
        m.template('qform_presenter_template.rb', "vendor/plugins/qtonrails/app/presenters/#{lowered_class_name}_qform_presenter.rb", :assigns => {:proxy_file => ui_proxy_filename, :model_keys => model_keys(eval(class_name).new) } )
